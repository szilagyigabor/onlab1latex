...
\section{Antennák jellemzői, definíciók}
	...
	\subsection{Iránykarakterisztika, irányhatás és nyereség}
		...
		\par Sajnos a $Q$ fenti definíciója szerinti meghatározása antennákra tulajdonképpen nem lehetséges, még szimuláció esetén is nagyon nehézkes, nemhogy mérés által. Ellenben a koncentrált paraméterű rezgőkörökkel, ahol az induktív komponensekben tárolt elektromos vagy mágneses energia és a legtöbbször ohmos vagy dielektromos veszteség jól kezelhető, antennákra ez korántsem mondható el. Emiatt különböző közelítő képleteket szokás használni antennák $Q$-jának becslésére, amik általában ekvivalens koncentrált paraméterű rezgőkörök $Q$-jára vezetik vissza az antenna $Q$-ját.
		...
		\begin{align}
			...
			\end{split}
		\end{align}
		ahol $f_0$ a rezonanciafrekvencia, $\Delta f$ pedig annak a két frekvenciának a különbsége $f_0$ két oldalán, amiken a rezonátorban maximálisan tárolt energia éppen a fele a rezonanciafrekvencián maximálisan tárolt energiának (3 dB sávszélesség). Röviden a második definíció szerint $Q$ egy rezonátor relatív sávszélességének a reciproka. Ennél a definíciónál viszont felmerül a sávszélességet meghatározó arányszám (jelen esetben 3 dB) megválasztásának kérdése.
		\par Egy másik, valamivel szofisztikáltabb közelítés a rezonáns antenna $Q$-ját a reflexiós tényező frekvencia szerinti deriváltja segítségével közelíti, \aref{equ:Q3}. egyenlet szerint. 
		\begin{align}
			\begin{split}\label{equ:Q3}
				Q_3 & = \omega \frac{\vardelta |\rho(\omega)|}{\delta \omega},
			\end{split}
		\end{align}
		\par Egy antenna $Q$-ját nem lehet ügyes tervezéssel minden határon túl csökkenteni, mivel az antenna működési hullámhosszra normált mérete meghatároz egy elméleti minimumot a $Q$-jára vonatkozóan, de ennek az alsó határnak a minél pontosabb meghatározása az elmúlt évtizedekben népszerű kutatási téma volt \cite{msa}.
